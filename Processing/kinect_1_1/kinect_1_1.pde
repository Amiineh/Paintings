...
void setup() {
        ...
        lines = new Line[numLines]; // Create the array
...
void drawSkeleton(int userId) {
        ...
        //drawCircle(userId, SimpleOpenNI.SKEL_RIGHT_HAND, 1);
...
void drawCircle(int userId, int jointID, int colorId){
      ...
      ellipse(width/2 + convertedJoint.x, convertedJoint.y, r, r);
}


void drawRing(int userId, int jointID, int colorId){
  PVector joint = new PVector();
  float confidence = kinect.getJointPositionSkeleton(userId, jointID, joint);
  ...
  rings[currentRing].start(convertedJoint.x, convertedJoint.y);
  currentRing++;
  if (currentRing >= numRings) {
    currentRing = 0;
  }
  println(convertedJoint.x, convertedJoint.y);
  
  fill(255, 0, 0);
  ellipse(width/2 + convertedJoint.x, convertedJoint.y, 20, 20);
}
...
void drawLine(int userId, int jointID1, int jointID2){
      ...
      lines[currentLine].start(convertedJoint1.x, convertedJoint1.y, convertedJoint2.x, convertedJoint2.y);
